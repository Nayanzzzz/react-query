...
import { addFormSchema } from "../../add/_components/add-form/add-form-schema"; 
import EmployeeForm from "../../_components/empoly-form";
...
const EditForm = () => {
  ...
  // Get employee data from React Query cache
  const { data: employeeList } = useQuery({
    queryKey: ["employ"],
    queryFn: () => [],
    staleTime: Infinity, // Use cached data
  });
  ...
  useEffect(() => {
    if (employeeList) {
      ...
      if (employee) {
        ...
        // Employee not found, redirect back to listing
        router.push("/employ-listing");
      }
    }
  }, [employeeList, employeeId, router]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: (updatedEmployee) => {
      ...
      queryClient.setQueryData(["employ"], (oldList) => {
        if (!oldList) return [updatedEmployee];
  ...
  const handleSubmit = (values) => {
    const updatedEmployee = {
      id: employeeId,
      ...addFormSchema.parse(values),
    };
    
    mutation.mutate(updatedEmployee);
  };

  // Wait until we have the employee data before rendering the form
  if (!defaultValues) {
    return <div className="p-4 mx-auto text-center">Loading...</div>;
  }
